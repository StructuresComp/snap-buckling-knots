...
void elasticRod::calundeformedTwist()
{
	Vector3d m1(0, 0, 1);
  MatrixXd M1(300,3);
  Vector3d t1 = tangent.row(0);
  Vector3d t2;
  Vector3d m1_1;
	for (int i = 0; i< ne; i++)
	{
		t2 = tangent.row(i);
		parallelTansport(m1, t1, t2, m1_1);
	...
	for (int i = 0; i<ne; i++)
	{
		m1 = M1.row(i);
		t2 = tangent.row(i);
        parallelTansport(m1, t2, t1, m1_1);
	...
	for (int i = 1; i<ne; i++)
	{
		undeformedTwist(i) = signedAngle(M1.row(i-1), M1.row(i), t1);
	}
}
...
void elasticRod::setMass()
{
	...
	for (int i=0; i<nv; i++)
	{
		...
		{
			massArray[4*i+k] = dm;
			inv_M(4*i +k, 4 *i +k) = 1/dm;
		}

		if (i < nv-1)
		{
			massArray[4*i+3] = rodRadius * rodRadius * dm / 2.0;
			inv_M(4*i + 3, 4 *i +3) = 1/dm;
		}
	}
...
void elasticRod::updateNewtonX(double *dx, double a)
{
	for (int c=0; c < uncons; c++)
	{
		x[unconstrainedMap[c]] -= a *dx[c];
	}
}
